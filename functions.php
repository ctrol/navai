...
function navai_ai_tool_meta_box_callback($post) {
    ...
    <div class="navai-meta-wrap">
        ...
        <div class="navai-field">
            <label>远程图片采集</label>
            <p class="description" style="margin-bottom:8px;">自动扫描并采集文章内容中的外部图片到本地媒体库</p>
            ...
            <div id="navai-image-list" style="margin-top:12px;display:none;">
                <p style="font-weight:600;margin-bottom:6px;">发现以下外链图片：</p>
            ...
            <div id="navai-download-progress" style="margin-top:8px;display:none;">
                ...
                <p id="navai-download-status" class="description" style="margin-top:4px;">准备中...</p>
            </div>
        </div>
    ...
    <div class="wrap">
        ...
        <form method="post" action="" enctype="multipart/form-data">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th><label for="content_ad"><?php _e('详情页正文广告代码', 'navai'); ?></label></th>
                    <td>
                        <textarea id="content_ad" name="content_ad" rows="6" cols="50" class="large-text code"><?php echo esc_textarea($content_ad); ?></textarea>
                        <p class="description"><?php _e('在详情页网站简介与缩略图之间显示的广告代码，支持HTML/JS（如百度联盟、Google AdSense等），建议尺寸 320x210', 'navai'); ?></p>
                    </td>
                </tr>
                <tr>
...
function navai_ajax_scan_remote_images() {
    ...
    if (empty($content)) {
        wp_send_json_error(array('message' => '内容为空'));
    }
    ...
    wp_send_json_success(array(
        'images' => $images,
        'count'  => count($images),
        'message'  => count($images) > 0 ? '发现 ' . count($images) . ' 张外部图片' : '未发现外部图片',
    ));
}
...
function navai_ajax_download_remote_images() {
    ...
    if (empty($images)) {
        wp_send_json_error(array('message' => '没有需要下载的图片'));
    }
    ...
    foreach ($images as $idx => $img) {
        ...
        if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
            $failed++;
            $results[] = array('url' => $old_url, 'status' => 'failed', 'reason' => '下载失败');
            continue;
        }
        ...
        if (empty($image_data)) {
            $failed++;
            $results[] = array('url' => $old_url, 'status' => 'failed', 'reason' => '内容为空');
            continue;
        }
        ...
        if ($saved === false) {
            $failed++;
            $results[] = array('url' => $old_url, 'status' => 'failed', 'reason' => '保存失败');
            continue;
        }

        $attachment = array(
            'post_title'     => '远程图片 - ' . ($idx + 1),
            'post_content'   => '',
            'post_status'    => 'inherit',
        ...
        if (is_wp_error($attach_id)) {
            $failed++;
            @unlink($file_path);
            $results[] = array('url' => $old_url, 'status' => 'failed', 'reason' => '媒体库插入失败');
            continue;
        }
    ...
    wp_send_json_success(array(
        ...
        'results'     => $results,
        'new_content' => $content,
        'message'     => '采集完成：成功 ' . $downloaded . ' 张，失败 ' . $failed . ' 张，共 ' . count($images) . ' 张',
    ));
}
    ...
    (function($) {
        ...
        $(document).ready(function() {
            ...
            $fetchBtn.on("click", function(e) {
                ...
                $fetchBtn.html('<span class="dashicons dashicons-update-alt spinning" style="font-size:16px;width:16px;height:16px;line-height:1;"></span> 采集中...');
            ...
            $(document).on("click", "#navai-scan-remote-images", function() {
                ...
                if (!content || content.length < 10) {
                    alert("编辑器内容为空，请先输入内容");
                    return;
                }

                $btn.prop("disabled", true);
                var origHtml = $btn.html();
                $btn.html('<span class="dashicons dashicons-update-alt spinning" style="font-size:16px;width:16px;height:16px;line-height:1;"></span> 扫描中...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $status.text("采集失败：" + (response.data.message || "未知错误"));
