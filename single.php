<?php
/**
 * 单篇文章模板文件
 *
 * @package NavAi
 * @author 老九
 * @version 1.29.22
 */

// 防止直接访问
if ( ! defined('ABSPATH')) {
	exit;
}

get_header();

// 确保全局 $post 对象已设置
if (have_posts()) {
	the_post();
}

$post_id   = get_the_ID();
$post_type = get_post_type();

// 初始化默认值
$website_url   = '';
$site_icon_url = '';
$is_hot        = false;
$is_new        = false;
$categories    = false;

// 如果是AI工具类型
if ('ai_tool' === $post_type ) {
	$website_url   = get_post_meta($post_id, '_website_url', true);
	$site_icon_url = get_post_meta($post_id, '_site_icon_url', true);
	$is_hot        = get_post_meta($post_id, '_is_hot', true);
	$is_new        = get_post_meta($post_id, '_is_new', true);
	$categories    = get_the_terms($post_id, 'ai_category');
}

$thumbnail = get_the_post_thumbnail_url($post_id, 'large');

// 评分数据
$rating_total   = (int) get_post_meta($post_id, '_rating_total', true);
$rating_count   = (int) get_post_meta($post_id, '_rating_count', true);
$avg_rating     = $rating_count > 0 ? round($rating_total / $rating_count, 1) : 0;
$bookmark_count = (int) get_post_meta($post_id, '_bookmark_count', true);

// 获取排行榜数量
$ranking_count = get_option('navai_ranking_count', 10);

// 获取排行榜数据（按浏览量排序）
$ranking_args = array(
	'post_type'      => 'ai_tool',
	'posts_per_page' => $ranking_count,
	'post__not_in'   => array($post_id),
	'meta_key'       => '_post_views',
	'orderby'        => 'meta_value_num',
	'order'          => 'DESC',
);
$ranking_query = new WP_Query($ranking_args);
// 如果没有浏览量数据，按发布日期排序
if (!$ranking_query->have_posts()) {
	$ranking_args = array(
		'post_type'      => 'ai_tool',
		'posts_per_page' => $ranking_count,
		'post__not_in'   => array($post_id),
		'orderby'        => 'date',
		'order'          => 'DESC',
	);
	$ranking_query = new WP_Query($ranking_args);
}
?>

<div class="main-content detail-page" style="flex: 1;">

	<!-- 顶部信息栏 -->
	<div class="detail-header">
		<div class="detail-header-main">
			<h1 class="detail-title"><?php the_title(); ?></h1>

			<div class="detail-meta">
				<span class="detail-meta-item">
					<i data-lucide="clock"></i>
					<?php echo get_the_date(); ?><?php esc_html_e('发布', 'navai'); ?>
				</span>
				<?php if ($avg_rating > 0) : ?>
				<span class="detail-meta-item navai-detail-rating" data-avg="<?php echo esc_attr($avg_rating); ?>" data-total="<?php echo esc_attr($rating_total); ?>" data-count="<?php echo esc_attr($rating_count); ?>">
					<i data-lucide="star" style="width:14px;height:14px;color:#f59e0b;"></i>
					<?php echo esc_html($avg_rating); ?><small>(<?php echo esc_html($rating_count); ?>)</small>
				</span>
				<?php endif; ?>
				<?php if ($bookmark_count > 0) : ?>
				<span class="detail-meta-item">
					<i data-lucide="bookmark" style="width:14px;height:14px;"></i>
					<?php echo esc_html($bookmark_count); ?>
				</span>
				<?php endif; ?>
			</div>

			<p class="detail-subtitle"><?php echo esc_html(wp_trim_words(get_the_excerpt(), 30)); ?></p>

			<div class="detail-info-table">
				<div class="detail-info-row">
					<div class="detail-info-label"><?php esc_html_e('收录时间：', 'navai'); ?></div>
					<div class="detail-info-value"><?php echo get_the_date('Y-m-d'); ?></div>
				</div>
			</div>

			<!-- 评分区域 -->
			<div class="navai-rating-section" data-post-id="<?php echo $post_id; ?>" style="margin-top:16px;padding:12px 16px;background:#f8fafc;border-radius:10px;">
				<div class="navai-rating-label" style="font-size:13px;color:#64748b;margin-bottom:8px;"><?php esc_html_e('评分：', 'navai'); ?></div>
				<div class="navai-rating-stars" style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
					<?php for ($r = 1; $r <= 5; $r++) : ?>
					<span class="navai-star star-<?php echo $r; ?>" data-value="<?php echo $r; ?>" style="font-size:24px;cursor:pointer;color:<?php echo $r <= round($avg_rating) ? '#f59e0b' : '#d1d5db'; ?>;transition:color .2s;" title="<?php echo $r; ?>"><?php echo $r <= round($avg_rating) ? '★' : '☆'; ?></span>
					<?php endfor; ?>
					<span class="navai-rating-result" style="margin-left:8px;font-size:14px;color:#334155;font-weight:600;"><?php echo $avg_rating > 0 ? esc_html($avg_rating) . ' / 5' : '<span style="color:#94a3b8;font-weight:400;">未评分</span>'; ?></span>
					<span class="navai-rating-count" style="margin-left:4px;font-size:12px;color:#94a3b8;"><?php echo $rating_count > 0 ? '(' . esc_html($rating_count) . ' ' . esc_html__('人投票', 'navai') . ')' : ''; ?></span>
				</div>
			</div>

			<?php if ( ! empty($website_url)) : ?>
			<div class="detail-actions">
				<a href="<?php echo esc_url($website_url); ?>" class="detail-btn-primary" target="_blank" rel="noopener noreferrer">
					<?php esc_html_e('打开网站', 'navai'); ?> <i data-lucide="external-link"></i>
				</a>
				<button type="button" class="detail-btn-secondary navai-bookmark-btn<?php echo navai_is_bookmarked($post_id) ? ' bookmarked' : ''; ?>" data-post-id="<?php echo $post_id; ?>" title="<?php esc_attr_e('收藏', 'navai'); ?>">
					<i data-lucide="bookmark" class="bookmark-outline" style="width:16px;height:16px;"></i>
					<span class="navai-bookmark-label"><?php echo navai_is_bookmarked($post_id) ? esc_html_e('已收藏', 'navai') : esc_html_e('收藏', 'navai'); ?></span>
					<?php if ($bookmark_count > 0) : ?><span class="navai-bookmark-count">(<?php echo esc_html($bookmark_count); ?>)</span><?php endif; ?>
				</button>
			</div>
			<?php endif; ?>

			<?php if ($categories && !is_wp_error($categories)) : ?>
			<div class="detail-tags">
				<?php foreach ($categories as $cat) : ?>
				<a href="<?php echo esc_url(get_term_link($cat)); ?>" class="detail-tag" rel="tag">
					<i data-lucide="folder"></i><?php echo esc_html($cat->name); ?>
				</a>
				<?php endforeach; ?>
			</div>
			<?php endif; ?>
		</div>

		<!-- 右侧预览图 + 广告 -->
		<div class="detail-preview-wrap">
			<!-- 广告位 -->
			<?php
			$content_ad = get_option('navai_content_ad', '');
			if ( ! empty($content_ad)) :
			?>
			<div class="detail-header-ad">
				<div class="detail-header-ad-inner">
					<?php echo wp_kses_post($content_ad); ?>
				</div>
			</div>
			<?php endif; ?>

			<!-- 右侧预览图 -->
			<div class="detail-preview">
				<div class="detail-preview-body">
					<div class="detail-favicon">
						<?php if ($site_icon_url) : ?>
							<img src="<?php echo esc_url($site_icon_url); ?>" alt="<?php echo esc_attr(get_the_title()); ?>" width="16" height="16">
						<?php else : ?>
							<i data-lucide="globe" style="width:16px;height:16px;"></i>
						<?php endif; ?>
						<span><?php echo esc_html(get_the_title()); ?></span>
					</div>
					<div class="detail-screenshot">
						<?php if ( ! empty($website_url)) : ?>
							<img src="https://s0.wp.com/mshots/v1/<?php echo esc_url($website_url); ?>?w=456&h=300" alt="<?php echo esc_attr(get_the_title() . ' ' . esc_attr__('网页截图', 'navai')); ?>" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" loading="lazy">
							<div class="detail-screenshot-placeholder" style="display:none;">
								<i data-lucide="image" style="width:48px;height:48px;color:var(--gray-300);"></i>
							</div>
						<?php elseif ($thumbnail) : ?>
							<img src="<?php echo esc_url($thumbnail); ?>" alt="<?php echo esc_attr(get_the_title() . ' ' . esc_attr__('网页截图', 'navai')); ?>">
						<?php else : ?>
							<div class="detail-screenshot-placeholder">
								<i data-lucide="image" style="width:48px;height:48px;color:var(--gray-300);"></i>
							</div>
						<?php endif; ?>
						<?php if ( ! empty($website_url)) : ?>
						<a href="<?php echo esc_url($website_url); ?>" class="detail-preview-btn" target="_blank" rel="noopener noreferrer">
							<?php esc_html_e('打开网站', 'navai'); ?>
						</a>
						<?php endif; ?>
					</div>
				</div>
			</div>
		</div>
	</div>

	<!-- 正文 + 右侧排行榜 -->
	<div class="detail-body">
		<div class="detail-main">
			<!-- 正文内容 -->
			<div class="detail-content">
				<div class="detail-content-body">
					<div class="detail-article">
						<?php the_content(); ?>
					</div>
				</div>
			</div>

			<!-- 相关推荐 -->
			<?php
			if ($categories && !is_wp_error($categories)) {
				$related_args = array(
					'post_type'      => 'ai_tool',
					'posts_per_page' => 8,
					'post__not_in'   => array($post_id),
					'tax_query'      => array(
						array(
							'taxonomy' => 'ai_category',
							'field'    => 'term_id',
							'terms'    => wp_list_pluck($categories, 'term_id'),
						),
					),
				);

				$related_query = new WP_Query($related_args);

				if ($related_query->have_posts()) :
			?>
			<div class="detail-related">
				<h3 class="detail-related-title">
					<i data-lucide="tag"></i>
					<?php printf(esc_html__('%s相关导航', 'navai'), esc_html(get_the_title())); ?>
				</h3>
				<div class="detail-related-grid">
					<?php while ($related_query->have_posts()) : $related_query->the_post(); ?>
						<?php
						$related_id      = get_the_ID();
						$related_url     = get_post_meta($related_id, '_website_url', true);
						$related_icon    = get_post_meta($related_id, '_site_icon_url', true);
						$related_thumb   = get_the_post_thumbnail_url($related_id, 'thumbnail');
						$related_excerpt = wp_trim_words(get_the_excerpt(), 15);
						?>
						<div class="detail-related-item" data-post-id="<?php echo esc_attr($related_id); ?>" title="<?php echo esc_attr($related_excerpt); ?>">
							<a href="<?php echo $related_url ? esc_url($related_url) : esc_url(get_permalink()); ?>"
                               class="detail-related-icon<?php if (!$related_icon && !$related_thumb) : ?> no-bg<?php endif; ?>"
                               target="_blank"
                               rel="noopener noreferrer">
								<?php if ($related_icon) : ?>
									<img src="<?php echo esc_url($related_icon); ?>" alt="<?php echo esc_attr(get_the_title()); ?>" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
									<span style="display:none;"><?php echo esc_html(mb_substr(get_the_title(), 0, 1)); ?></span>
								<?php elseif ($related_thumb) : ?>
									<img src="<?php echo esc_url($related_thumb); ?>" alt="<?php echo esc_attr(get_the_title()); ?>">
								<?php else : ?>
									<span><?php echo esc_html(mb_substr(get_the_title(), 0, 1)); ?></span>
								<?php endif; ?>
							</a>
							<a href="<?php echo esc_url(get_permalink()); ?>" class="detail-related-body">
								<h4 class="detail-related-name"><?php the_title(); ?></h4>
								<p class="detail-related-desc"><?php echo esc_html($related_excerpt); ?></p>
							</a>
						</div>
					<?php endwhile; ?>
				</div>
				<?php wp_reset_postdata(); ?>
			</div>
			<?php
				endif;
			}
			?>

			<!-- 评论区域 -->
			<?php if (comments_open() || get_comments_number()) : ?>
			<div class="detail-comments" id="comments">
				<?php comments_template(); ?>
			</div>
			<?php endif; ?>
		</div>

		<!-- 右侧浮动排行榜 -->
		<?php if ($ranking_query->have_posts()) : ?>
		<div class="detail-sidebar">
			<div class="detail-sidebar-sticky">
				<div class="detail-ranking">
					<div class="detail-ranking-header">
						<h3 class="detail-ranking-title">
							<i data-lucide="trophy"></i>
							<?php esc_html_e('排行榜', 'navai'); ?>
						</h3>
					</div>
					<div class="detail-ranking-list">
						<?php
						$rank = 1;
						while ($ranking_query->have_posts()) : $ranking_query->the_post();
							$rank_id     = get_the_ID();
							$rank_url    = get_post_meta($rank_id, '_website_url', true);
							$rank_icon   = get_post_meta($rank_id, '_site_icon_url', true);
							$rank_thumb  = get_the_post_thumbnail_url($rank_id, 'thumbnail');
							$rank_desc   = wp_trim_words(get_the_excerpt(), 20);
						?>
						<div class="detail-ranking-item" data-post-id="<?php echo esc_attr($rank_id); ?>" title="<?php echo esc_attr($rank_desc); ?>">
							<span class="detail-ranking-num"><?php echo $rank; ?></span>
							<a href="<?php echo $rank_url ? esc_url($rank_url) : esc_url(get_permalink()); ?>"
                               class="detail-ranking-icon<?php if (!$rank_icon && !$rank_thumb) : ?> no-bg<?php endif; ?>"
                               target="_blank"
                               rel="noopener noreferrer">
								<?php if ($rank_icon) : ?>
									<img src="<?php echo esc_url($rank_icon); ?>" alt="<?php echo esc_attr(get_the_title()); ?>" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
									<span style="display:none;"><?php echo esc_html(mb_substr(get_the_title(), 0, 1)); ?></span>
								<?php elseif ($rank_thumb) : ?>
									<img src="<?php echo esc_url($rank_thumb); ?>" alt="<?php echo esc_attr(get_the_title()); ?>">
								<?php else : ?>
									<span><?php echo esc_html(mb_substr(get_the_title(), 0, 1)); ?></span>
								<?php endif; ?>
							</a>
							<a href="<?php echo esc_url(get_permalink()); ?>" class="detail-ranking-info">
								<h4 class="detail-ranking-name"><?php the_title(); ?></h4>
								<p class="detail-ranking-desc"><?php echo esc_html($rank_desc); ?></p>
							</a>
						</div>
						<?php $rank++; endwhile; wp_reset_postdata(); ?>
					</div>
				</div>

				<!-- 侧边栏广告 -->
				<?php
				$sidebar_ad = get_option('navai_sidebar_ad', '');
				if ( ! empty($sidebar_ad)) :
				?>
				<div class="detail-sidebar-ad">
					<?php echo wp_kses_post($sidebar_ad); ?>
				</div>
				<?php endif; ?>
			</div>
		</div>
		<?php endif; ?>
	</div>

</div>

<?php get_sidebar(); ?>

<!-- 浮动式菜单 -->
<div class="floating-menu" id="floating-menu">
	<button class="floating-menu-toggle" id="floating-menu-toggle" aria-label="<?php esc_attr_e('打开菜单', 'navai'); ?>">
		<i data-lucide="plus"></i>
	</button>
	<div class="floating-menu-items">
		<?php if ( ! empty($website_url)) : ?>
		<a href="<?php echo esc_url($website_url); ?>" class="floating-menu-item" target="_blank" rel="noopener noreferrer" title="<?php esc_attr_e('访问网站', 'navai'); ?>">
			<i data-lucide="external-link"></i>
			<span class="floating-menu-label"><?php esc_html_e('访问网站', 'navai'); ?></span>
		</a>
		<?php endif; ?>
		<a href="javascript:void(0);" class="floating-menu-item" id="copy-link-btn" title="<?php esc_attr_e('复制链接', 'navai'); ?>">
			<i data-lucide="link"></i>
			<span class="floating-menu-label"><?php esc_html_e('复制链接', 'navai'); ?></span>
		</a>
		<a href="javascript:void(0);" class="floating-menu-item" id="share-btn" title="<?php esc_attr_e('分享', 'navai'); ?>">
			<i data-lucide="share-2"></i>
			<span class="floating-menu-label"><?php esc_html_e('分享', 'navai'); ?></span>
		</a>
		<a href="#comments" class="floating-menu-item" title="<?php esc_attr_e('评论', 'navai'); ?>">
			<i data-lucide="message-circle"></i>
			<span class="floating-menu-label"><?php esc_html_e('评论', 'navai'); ?></span>
		</a>
		<a href="#comments" class="floating-menu-item" title="<?php esc_attr_e('返回顶部', 'navai'); ?>" id="back-to-top-fab">
			<i data-lucide="chevron-up"></i>
			<span class="floating-menu-label"><?php esc_html_e('返回顶部', 'navai'); ?></span>
		</a>
	</div>
</div>

<?php get_footer(); ?>
<!-- NavAi 详情页评分/收藏脚本 -->
<script>
(function($) {
    // 详情页评分
    $(document).on('click', '.navai-star', function() {
        var $star = $(this);
        var $section = $star.closest('.navai-rating-section');
        var postId = $section.data('post-id');
        var rating = parseInt($star.data('value'));
        if (!postId) return;

        $star.siblings('.navai-rating-result').html('<span style="color:#94a3b8;">' + (navaiAjax.str_rated || '...') + '</span>');

        $.post(navaiAjax.ajaxurl, {
            action: 'navai_submit_rating',
            post_id: postId,
            rating: rating,
            nonce: navaiAjax.rating_nonce
        }, function(res) {
            if (res.success) {
                var avgRound = Math.round(res.data.avg);
                var $stars = $section.find('.navai-star');
                $stars.each(function(i) {
                    var filled = (i + 1) <= avgRound;
                    $(this).html(filled ? '★' : '☆').css('color', filled ? '#f59e0b' : '#d1d5db');
                });
                $section.find('.navai-rating-result').html(res.data.avg + ' / 5');
                $section.find('.navai-rating-count').text('(' + res.data.count + ' ' + (navaiAjax.str_votes || '') + ')');
            } else {
                $section.find('.navai-rating-result').html('<span style="color:#ef4444;">' + (res.data && res.data.message ? res.data.message : navaiAjax.str_error) + '</span>');
            }
        }).fail(function() {
            $section.find('.navai-rating-result').html('<span style="color:#ef4444;">' + navaiAjax.str_error + '</span>');
        });
    });

    // 详情页书签
    $(document).on('click', '.navai-bookmark-btn', function(e) {
        e.preventDefault();
        var $btn = $(this);
        var postId = $btn.data('post-id');
        if (!postId) return;

        $.post(navaiAjax.ajaxurl, {
            action: 'navai_toggle_bookmark',
            post_id: postId,
            nonce: navaiAjax.bookmark_nonce
        }, function(res) {
            if (res.success) {
                var $icon = $btn.find('i');
                if (res.data.is_bookmarked) {
                    $icon.removeClass('bookmark-outline').addClass('bookmark-filled');
                    $btn.addClass('bookmarked');
                    $btn.find('.navai-bookmark-label').text('已收藏');
                } else {
                    $icon.removeClass('bookmark-filled').addClass('bookmark-outline');
                    $btn.removeClass('bookmarked');
                    $btn.find('.navai-bookmark-label').text('收藏');
                }
                var $count = $btn.find('.navai-bookmark-count');
                if ($count.length) {
                    $count.text('(' + res.data.count + ')');
                } else if (res.data.count > 0) {
                    $btn.append('<span class="navai-bookmark-count">(' + res.data.count + ')</span>');
                }
            } else {
                alert(res.data && res.data.message ? res.data.message : navaiAjax.str_error);
            }
        }).fail(function() {
            alert(navaiAjax.str_error);
        });
    });
})(jQuery);
</script>
